...
import { BitgetRestClient, BitgetApiError, buildTools, getEarnCapabilityStatus, toToolErrorPayload, warmupEarnCapability, MODULES, SERVER_NAME, SERVER_VERSION } from 'bitget-core';
...
function successResult(toolName: string, data: unknown, capabilitySnapshot: CapabilitySnapshot): CallToolResult {
...
function errorResult(toolName: string, error: unknown, capabilitySnapshot: CapabilitySnapshot): CallToolResult {
...
function unknownToolResult(toolName: string, capabilitySnapshot: CapabilitySnapshot): CallToolResult {
  return errorResult(toolName, new BitgetApiError(`Tool "${toolName}" is not available in this server session.`, {
