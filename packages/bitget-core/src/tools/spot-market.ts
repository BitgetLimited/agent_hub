...
import { assertEnum, compactObject, readNumber, readString, asRecord } from './helpers.js';
...
export function registerSpotMarketTools(): ToolSpec[] {
  return [{
    ...
    handler: async (rawArgs, context) => {
      ...
      const response = await context.client.publicGet(SPOT_ENDPOINTS.market.tickers, compactObject({ symbol }), publicRateLimit('spot_get_ticker', 20));
    ...
    handler: async (rawArgs, context) => {
      ...
      const path =
        type === 'step0'
          ? SPOT_ENDPOINTS.market.orderbook
          : SPOT_ENDPOINTS.market.mergeDepth;
      ...
      const response = await context.client.publicGet(path, compactObject({ symbol, precision, limit }), publicRateLimit('spot_get_depth', 20));
      return normalize(response);
    }
  }, {
    ...
    handler: async (rawArgs, context) => {
      ...
      const path = startTime
        ? SPOT_ENDPOINTS.market.candles
        : SPOT_ENDPOINTS.market.historyCandles;
      const response = await context.client.publicGet(path, compactObject({ symbol, granularity, startTime, endTime, limit }), publicRateLimit('spot_get_candles', 20));
      return normalize(response);
    }
  }, {
    ...
    handler: async (rawArgs, context) => {
      ...
      const endTime = readString(args, 'endTime');
      const path = startTime
        ? SPOT_ENDPOINTS.market.fillsHistory
        : SPOT_ENDPOINTS.market.fills;
      const response = await context.client.publicGet(path, compactObject({ symbol, limit, startTime, endTime }), publicRateLimit('spot_get_trades', 10));
      return normalize(response);
    }
  }, {
    ...
    handler: async (rawArgs, context) => {
      ...
      const path =
        type === 'coins'
          ? SPOT_ENDPOINTS.market.coins
          : SPOT_ENDPOINTS.market.symbols;
      const response = await context.client.publicGet(path, compactObject({ symbol, coin }), publicRateLimit('spot_get_symbols', 20));
      return normalize(response);
    }
  }];
}
