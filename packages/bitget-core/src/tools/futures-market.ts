...
import { asRecord, assertEnum, compactObject, readBoolean, readNumber, readString, requireString } from './helpers.js';
...
export function registerFuturesMarketTools(): ToolSpec[] {
  return [{
    ...
    handler: async (rawArgs, context) => {
      ...
      const path = symbol
        ? FUTURES_ENDPOINTS.market.ticker
        : FUTURES_ENDPOINTS.market.tickers;
      const response = await context.client.publicGet(path, compactObject({ productType, symbol }), publicRateLimit('futures_get_ticker', 20));
    ...
    handler: async (rawArgs, context) => {
      ...
      assertEnum(productType, 'productType', PRODUCT_TYPES);
      const response = await context.client.publicGet(FUTURES_ENDPOINTS.market.mergeDepth, compactObject({
          productType,
          symbol,
    ...
    handler: async (rawArgs, context) => {
      ...
      if (priceType === 'index' && startTime) {
        path = FUTURES_ENDPOINTS.market.historyIndexCandles;
        ...
        path = FUTURES_ENDPOINTS.market.historyMarkCandles;
        queryParams = compactObject({ productType, symbol, granularity: apiGranularity, startTime, endTime, limit });
      } else if (priceType === 'trade' && startTime) {
        path = FUTURES_ENDPOINTS.market.historyCandles;
        queryParams = compactObject({ productType, symbol, granularity: apiGranularity, startTime, endTime, limit });
      } else if (priceType === 'index') {
        path = FUTURES_ENDPOINTS.market.candles;
      ...
      const response = await context.client.publicGet(path, queryParams, publicRateLimit('futures_get_candles', 20));
      return normalize(response);
    }
  }, {
    ...
    handler: async (rawArgs, context) => {
      ...
      const path = startTime
        ? FUTURES_ENDPOINTS.market.fillsHistory
        : FUTURES_ENDPOINTS.market.fills;
      const response = await context.client.publicGet(path, compactObject({
          productType,
          symbol,
          limit: readNumber(args, 'limit'),
          startTime,
    ...
    handler: async (rawArgs, context) => {
      ...
      const productType = requireString(args, 'productType');
      assertEnum(productType, 'productType', PRODUCT_TYPES);
      const response = await context.client.publicGet(FUTURES_ENDPOINTS.market.contracts, compactObject({
          productType,
    ...
    handler: async (rawArgs, context) => {
      ...
      if (history) {
        const historyResponse = await context.client.publicGet(FUTURES_ENDPOINTS.market.historyFundRate, compactObject({
            productType,
            symbol,
      ...
      const fundingResults = await Promise.all([
        context.client.publicGet(FUTURES_ENDPOINTS.market.currentFundRate, compactObject({ productType, symbol }), publicRateLimit('futures_get_funding_rate_current', 20)),
        context.client.publicGet(FUTURES_ENDPOINTS.market.fundingTime, compactObject({ productType, symbol }), publicRateLimit('futures_get_funding_rate_time', 20))
      ]);

      return {
        endpoint: `${fundingResults[0].endpoint} + ${fundingResults[1].endpoint}`,
        requestTime: new Date().toISOString(),
        data: {
          currentFundRate: fundingResults[0].data,
          fundingTime: fundingResults[1].data
        }
      };
    }
  }, {
    ...
    handler: async (rawArgs, context) => {
      ...
      const symbol = requireString(args, 'symbol');
      assertEnum(productType, 'productType', PRODUCT_TYPES);
      const response = await context.client.publicGet(FUTURES_ENDPOINTS.market.openInterest, compactObject({ productType, symbol }), publicRateLimit('futures_get_open_interest', 20));
      return normalize(response);
    }
  }];
}
