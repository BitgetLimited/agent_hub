...
import { asRecord, assertEnum, compactObject, readNumber, readString, requireString } from './helpers.js';
import { privateRateLimit } from './common.js';
import { BROKER_ENDPOINTS } from '../api/index.js';
...
export function registerBrokerTools(): ToolSpec[] {
  return [{
    ...
    handler: async (_rawArgs, context) => {
      const response = await context.client.privateGet(BROKER_ENDPOINTS.accountInfo, undefined, privateRateLimit('broker_get_info', 10));
    ...
    handler: async (rawArgs, context) => {
      ...
      if (action === 'list') {
        ...
        const response = await context.client.privateGet(BROKER_ENDPOINTS.subaccountList, payload, privateRateLimit('broker_manage_subaccounts', 5));
        return normalize(response);
      }
      if (action === 'create') {
        const payload = compactObject({
        ...
        const response = await context.client.privatePost(BROKER_ENDPOINTS.createSubaccount, payload, privateRateLimit('broker_manage_subaccounts', 5));
        return normalize(response);
      }
      // modify
      ...
      const response = await context.client.privatePost(BROKER_ENDPOINTS.modifySubaccount, payload, privateRateLimit('broker_manage_subaccounts', 5));
      return normalize(response);
    }
  }, {
    ...
    handler: async (rawArgs, context) => {
      ...
      if (action === 'list') {
        const response = await context.client.privateGet(BROKER_ENDPOINTS.apikeyList, compactObject({ subUid }), privateRateLimit('broker_manage_apikeys', 5));
        return normalize(response);
      }
      if (action === 'delete') {
        const response = await context.client.privatePost(BROKER_ENDPOINTS.deleteApikey, compactObject({ subUid, apiKey }), privateRateLimit('broker_manage_apikeys', 5));
        return normalize(response);
      }
      const endpoint =
        action === 'create'
          ? BROKER_ENDPOINTS.createApikey
          : BROKER_ENDPOINTS.modifyApikey;
      const payload = compactObject({
        subUid,
      ...
      const response = await context.client.privatePost(endpoint, payload, privateRateLimit('broker_manage_apikeys', 5));
      return normalize(response);
    }
  }];
}
