...
import { asRecord, assertEnum, compactObject, readNumber, readString, readStringArray, requireString } from './helpers.js';
import { privateRateLimit, PRODUCT_TYPES } from './common.js';
import { ACCOUNT_ENDPOINTS } from '../api/index.js';
...
export function registerAccountTools(): ToolSpec[] {
  return [{
    ...
    handler: async (rawArgs, context) => {
      ...
      const route =
        accountType === 'spot'
          ? ACCOUNT_ENDPOINTS.assets.spot
          : accountType === 'futures'
            ? ACCOUNT_ENDPOINTS.assets.futures
            : accountType === 'funding'
              ? ACCOUNT_ENDPOINTS.assets.funding
              : ACCOUNT_ENDPOINTS.assets.all;
      const response = await context.client.privateGet(route, compactObject({ coin, productType }), privateRateLimit('get_account_assets', 10));
    ...
    handler: async (rawArgs, context) => {
      ...
      const route =
        accountType === 'futures'
          ? ACCOUNT_ENDPOINTS.bills.futures
          : ACCOUNT_ENDPOINTS.bills.spot;
      const response = await context.client.privateGet(route, compactObject({
    ...
    handler: async (rawArgs, context) => {
      ...
      const path = isSubTransfer
        ? ACCOUNT_ENDPOINTS.wallet.subaccountTransfer
        : ACCOUNT_ENDPOINTS.wallet.transfer;
      const response = await context.client.privatePost(path, compactObject({
    ...
    handler: async (rawArgs, context) => {
      ...
      const response = await context.client.privatePost(ACCOUNT_ENDPOINTS.wallet.withdrawal, compactObject({
          coin: requireString(args, 'coin'),
          transferType,
    ...
    handler: async (rawArgs, context) => {
      const args = asRecord(rawArgs);
      const response = await context.client.privatePost(ACCOUNT_ENDPOINTS.wallet.cancelWithdrawal, { orderId: requireString(args, 'orderId') }, privateRateLimit('cancel_withdrawal', 10));
      return normalize(response);
    }
  }, {
    ...
    handler: async (rawArgs, context) => {
      const args = asRecord(rawArgs);
      const response = await context.client.privateGet(ACCOUNT_ENDPOINTS.wallet.depositAddress, compactObject({
          coin: requireString(args, 'coin'),
          chain: readString(args, 'chain')
    ...
    handler: async (rawArgs, context) => {
      ...
      const path =
        recordType === 'deposit'
          ? ACCOUNT_ENDPOINTS.wallet.depositRecords
          : recordType === 'withdrawal'
            ? ACCOUNT_ENDPOINTS.wallet.withdrawalRecords
            : ACCOUNT_ENDPOINTS.records.subMainTransRecord;
    ...
    handler: async (rawArgs, context) => {
      ...
      if (action === 'list') {
        const response = await context.client.privateGet(ACCOUNT_ENDPOINTS.subaccount.list, compactObject({ subAccountUid }), privateRateLimit('manage_subaccounts', 5));
        return normalize(response);
      }

      if (action === 'listApiKeys') {
        const response = await context.client.privateGet(ACCOUNT_ENDPOINTS.subaccount.apikeyList, compactObject({ subAccountUid }), privateRateLimit('manage_subaccounts', 5));
        return normalize(response);
      }

      if (action === 'create') {
        const response = await context.client.privatePost(ACCOUNT_ENDPOINTS.subaccount.create, compactObject({
      ...
      if (action === 'modify') {
        const response = await context.client.privatePost(ACCOUNT_ENDPOINTS.subaccount.modify, compactObject({
            subAccountUid,
      ...
      const endpoint =
        action === 'createApiKey'
          ? ACCOUNT_ENDPOINTS.subaccount.createApikey
          : ACCOUNT_ENDPOINTS.subaccount.modifyApikey;
      const response = await context.client.privatePost(endpoint, compactObject({
          subAccountUid,
