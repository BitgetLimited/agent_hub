...
import { asRecord, assertEnum, compactObject, ensureOneOf, readNumber, readString, readStringArray, requireString } from './helpers.js';
import { privateRateLimit } from './common.js';
import { CONVERT_ENDPOINTS } from '../api/index.js';
...
export function registerConvertTools(): ToolSpec[] {
  return [{
    ...
    handler: async (rawArgs, context) => {
      ...
      const path = fromCoin
        ? CONVERT_ENDPOINTS.quotedPrice
        : CONVERT_ENDPOINTS.currencies;
      ...
      if (fromCoin && toCoinSize && fromCoinSize) {
        throw new ValidationError('Provide only one of "fromCoinAmount" or "toCoinAmount".');
      }
      if (fromCoin && !readString(args, 'toCoin')) {
        throw new ValidationError('Parameter "toCoin" is required when "fromCoin" is provided.');
      }
    ...
    handler: async (rawArgs, context) => {
      ...
      const path =
        type === 'bgb'
          ? CONVERT_ENDPOINTS.bgbConvert
          : CONVERT_ENDPOINTS.trade;
      ...
      ensureOneOf(args, ['fromCoinAmount', 'toCoinAmount'], 'Provide one of "fromCoinAmount" or "toCoinAmount" for normal conversion.');
      ...
      const quoted = await context.client.privateGet<{
        ...
        cnvtPrice: string;
        traceId: string;
      }>(CONVERT_ENDPOINTS.quotedPrice, compactObject({ fromCoin, toCoin, fromCoinSize, toCoinSize }), privateRateLimit('convert_execute_quote', 10));
    ...
    handler: async (rawArgs, context) => {
      ...
      const path =
        type === 'bgb'
          ? CONVERT_ENDPOINTS.bgbConvertRecords
          : CONVERT_ENDPOINTS.convertRecord;
      const response = await context.client.privateGet(path, compactObject({
          startTime:
            type === 'normal'
