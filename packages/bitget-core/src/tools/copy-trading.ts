...
import { COPY_ENDPOINTS } from '../api/index.js';
  ...
  return [{
    ...
    handler: async (rawArgs, context) => {
      ...
      const path = isSpot(productType)
        ? COPY_ENDPOINTS.spot.queryTraders
        : COPY_ENDPOINTS.mix.queryTraders;
      const query = isSpot(productType)
        ? compactObject({ symbol, limit })
        : compactObject({ productType, symbol, limit });
      const response = await context.client.privateGet(path, query, privateRateLimit('copy_get_traders', 10));
    ...
    handler: async (rawArgs, context) => {
      ...
      if (!resolvedTraderId && autoSelectTrader) {
        const tradersPath = isSpot(productType)
          ? COPY_ENDPOINTS.spot.queryTraders
          : COPY_ENDPOINTS.mix.queryTraders;
        const tradersResponse = await context.client.privateGet(tradersPath, isSpot(productType)
            ? compactObject({
        ...
        if (!picked) {
          throw new BitgetApiError('No available copy traders found for auto selection.', {
      ...
      if (!resolvedTraderId) {
        throw new ValidationError('Missing "traderId". Provide traderId or set autoSelectTrader=true.');
      }
      ...
      if (isSpot(productType)) {
        ...
        if (dryRun) {
          return {
            endpoint: `DRY_RUN POST ${COPY_ENDPOINTS.spot.settings}`,
        ...
        const response = await context.client.privatePost(COPY_ENDPOINTS.spot.settings, payload, privateRateLimit('copy_place_order', 10));
        return {
          ...normalize(response),
          selection: {
            requestedTraderId: requestedTraderId ?? null,
      ...
      const symbol = readString(args, 'symbol');
      if (!symbol) {
        throw new ValidationError('Parameter "symbol" is required for futures copy settings.');
      }
      ...
      if (dryRun) {
        return {
          endpoint: `DRY_RUN POST ${COPY_ENDPOINTS.mix.settings}`,
          requestTime: new Date().toISOString(),
          data: {
            dryRun: true,
            payload,
      ...
      const response = await context.client.privatePost(COPY_ENDPOINTS.mix.settings, payload, privateRateLimit('copy_place_order', 10));
      return {
        ...normalize(response),
        selection: {
          requestedTraderId: requestedTraderId ?? null,
    ...
    handler: async (rawArgs, context) => {
      ...
      const response = await context.client.privatePost(COPY_ENDPOINTS.mix.closePositions, compactObject({
          productType,
    ...
    handler: async (rawArgs, context) => {
      ...
      assertEnum(productType, 'productType', COPY_PRODUCT_TYPES);
      const path = isSpot(productType)
        ? COPY_ENDPOINTS.spot.queryHistoryOrders
        : COPY_ENDPOINTS.mix.queryHistoryOrders;
      const query = isSpot(productType)
        ? compactObject({
            symbol: readString(args, 'symbol'),
      ...
      const response = await context.client.privateGet(path, query, privateRateLimit('copy_get_orders', 10));
      return normalize(response);
    }
  }, {
    ...
    handler: async (rawArgs, context) => {
      ...
      const path = isSpot(productType)
        ? history
          ? COPY_ENDPOINTS.spot.queryHistoryOrders
          : COPY_ENDPOINTS.spot.queryCurrentOrders
        : history
          ? COPY_ENDPOINTS.mix.queryHistoryOrders
          : COPY_ENDPOINTS.mix.queryCurrentOrders;
      const query = isSpot(productType)
        ? compactObject({
            symbol: readString(args, 'symbol'),
            startTime: readString(args, 'startTime'),
      ...
      const response = await context.client.privateGet(path, query, privateRateLimit('copy_get_positions', 10));
      return normalize(response);
    }
  }];
}
