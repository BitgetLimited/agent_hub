...
import { asRecord, assertEnum, compactObject, ensureOneOf, readBoolean, readNumber, readString, readStringArray, requireString } from './helpers.js';
import { privateRateLimit } from './common.js';
import { MARGIN_ENDPOINTS, marginEndpoint } from '../api/index.js';
...
export function registerMarginTools(): ToolSpec[] {
  return [{
    ...
    handler: async (rawArgs, context) => {
      ...
      const response = await context.client.privateGet(marginEndpoint(marginType, MARGIN_ENDPOINTS.assets), compactObject({
    ...
    handler: async (rawArgs, context) => {
      ...
      const marginType = requireString(args, 'marginType');
      assertEnum(marginType, 'marginType', MARGIN_TYPES);
      const response = await context.client.privatePost(marginEndpoint(marginType, MARGIN_ENDPOINTS.borrow), compactObject({
    ...
    handler: async (rawArgs, context) => {
      ...
      const path = flashRepay
        ? marginEndpoint(marginType, MARGIN_ENDPOINTS.flashRepay)
        : marginEndpoint(marginType, MARGIN_ENDPOINTS.repay);
      const response = await context.client.privatePost(path, body, privateRateLimit('margin_repay', 10));
      return normalize(response);
    }
  }, {
    ...
    handler: async (rawArgs, context) => {
      ...
      const response = await context.client.privatePost(marginEndpoint(marginType, MARGIN_ENDPOINTS.placeOrder), compactObject({
          symbol: requireString(args, 'symbol'),
          side,
          orderType,
    ...
    handler: async (rawArgs, context) => {
      ...
      assertEnum(marginType, 'marginType', MARGIN_TYPES);
      ensureOneOf(args, ['orderId', 'orderIds'], 'Provide one of "orderId" or "orderIds".');
      ...
      const path = orderId
        ? marginEndpoint(marginType, MARGIN_ENDPOINTS.cancelOrder)
        : marginEndpoint(marginType, MARGIN_ENDPOINTS.batchCancelOrder);
      const response = await context.client.privatePost(path, orderId
    ...
    handler: async (rawArgs, context) => {
      ...
      const path =
        status === 'history' || orderId
          ? marginEndpoint(marginType, MARGIN_ENDPOINTS.historyOrders)
          : marginEndpoint(marginType, MARGIN_ENDPOINTS.openOrders);
    ...
    handler: async (rawArgs, context) => {
      ...
      const recordTypeSuffixMap = {
        borrow: MARGIN_ENDPOINTS.borrowHistory,
        repay: MARGIN_ENDPOINTS.repayHistory,
        interest: MARGIN_ENDPOINTS.interestHistory,
        liquidation: MARGIN_ENDPOINTS.liquidationHistory
      } as const;
      const suffix =
        recordTypeSuffixMap[recordType as keyof typeof recordTypeSuffixMap];
      const now = Date.now();
      const defaultStartTime = String(now - 30 * 24 * 60 * 60 * 1000);
      const response = await context.client.privateGet(marginEndpoint(marginType, suffix), compactObject({
          coin: readString(args, 'coin'),
          symbol: readString(args, 'symbol'),
