...
import { asRecord, compactObject, ensureOneOf, readBoolean, readNumber, readObjectArray, readString, readStringArray, requireObjectArray, requireString } from './helpers.js';
import { privateRateLimit } from './common.js';
import { SPOT_ENDPOINTS } from '../api/index.js';
...
export function registerSpotTradeTools(): ToolSpec[] {
  return [{
    ...
    handler: async (rawArgs, context) => {
      ...
      const path = isSingle
        ? SPOT_ENDPOINTS.trade.placeOrder
        : SPOT_ENDPOINTS.trade.batchOrders;
      const body = isSingle ? normalizedOrders[0] : { orderList: normalizedOrders };
      const response = await context.client.privatePost(path, body, privateRateLimit('spot_place_order', 10));
    ...
    handler: async (rawArgs, context) => {
      ...
      const cancelTarget = orderId
        ? {
            path: SPOT_ENDPOINTS.trade.cancelOrder,
            body: { symbol, orderId }
          }
        : orderIds
          ? {
              path: SPOT_ENDPOINTS.trade.batchCancelOrder,
              body: { symbol, orderList: orderIds!.map((id) => ({ orderId: id })) }
            }
          : cancelAll
            ? {
                path: SPOT_ENDPOINTS.trade.cancelSymbolOrder,
                body: { symbol }
              }
            : {
                path: SPOT_ENDPOINTS.trade.cancelOrder,
      ...
      const response = await context.client.privatePost(path, body, privateRateLimit('spot_cancel_orders', 10));
      return normalize(response);
    }
  }, {
    ...
    handler: async (rawArgs, context) => {
      ...
      if (!newPrice && !newSize && !newClientOid) {
        throw new ValidationError('Provide at least one of "newPrice", "newSize", or "newClientOid".');
      }
      const response = await context.client.privatePost(SPOT_ENDPOINTS.trade.cancelReplaceOrder, compactObject({
    ...
    handler: async (rawArgs, context) => {
      ...
      const route = orderId
        ? SPOT_ENDPOINTS.trade.orderInfo
        : status === 'history'
          ? SPOT_ENDPOINTS.trade.historyOrders
          : SPOT_ENDPOINTS.trade.unfilledOrders;
      ...
      const response = await context.client.privateGet(route, query, privateRateLimit('spot_get_orders', 10));
      return normalize(response);
    }
  }, {
    name: 'spot_get_fills',
    ...
    handler: async (rawArgs, context) => {
      const args = asRecord(rawArgs);
      const response = await context.client.privateGet(SPOT_ENDPOINTS.trade.fills, compactObject({
    ...
    handler: async (rawArgs, context) => {
      ...
      const orderId = readString(args, 'orderId');
      const path = orderId
        ? SPOT_ENDPOINTS.trade.modifyPlanOrder
        : SPOT_ENDPOINTS.trade.placePlanOrder;
    ...
    handler: async (rawArgs, context) => {
      ...
      const path =
        status === 'history'
          ? SPOT_ENDPOINTS.trade.historyPlanOrder
          : SPOT_ENDPOINTS.trade.currentPlanOrder;
    ...
    handler: async (rawArgs, context) => {
      ...
      const orderId = readString(args, 'orderId');
      const symbol = readString(args, 'symbol');
      ensureOneOf(args, ['orderId', 'symbol'], 'Provide one of "orderId" or "symbol".');
      const path = orderId
        ? SPOT_ENDPOINTS.trade.cancelPlanOrder
        : SPOT_ENDPOINTS.trade.batchCancelPlanOrder;
      const body = orderId
        ? { orderId }
        : { symbolList: [symbol] };
      const response = await context.client.privatePost(path, body, privateRateLimit('spot_cancel_plan_orders', 10));
      return normalize(response);
    }
  }];
}
